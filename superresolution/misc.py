...
def get_subset_and_snippet(cell, data, image):
    ...
    mask = [cv2.pointPolygonTest(cell.hull, (point_x, point_y), measureDist=False)>=0 for point_x, point_y in zip(subset.x, subset.y)]
    subset = subset[mask]
    ...
    subset['rot_x'] = (subset.new_x-mid_x) * numpy.cos(corr_angle) - (subset.new_y-mid_y) * numpy.sin(corr_angle) + mid_x
    subset['rot_y'] = (subset.new_y-mid_y) * numpy.cos(corr_angle) + (subset.new_x-mid_x) * numpy.sin(corr_angle) + mid_y
...
def markerize_identical_emitters(cell):
    ...
    xy, precision, frame = numpy.c_[numpy.array(cell.subset.x), numpy.array(cell.subset.y)], numpy.array(cell.subset.locprec_x), numpy.array(cell.subset.frame)
...
def main():
    ...
    with PdfPages("%s" % (filename.output,)) as pdf:
        ...
        for cell in cells:
            cv2.putText(canvas, ". %d" % (cell.count,), tuple(map(int, cv2.minAreaRect(cell.hull)[0])), cv2.FONT_HERSHEY_PLAIN, 1, (255,0,0))
        ...
        for n, cell in enumerate(cells):
            ...
            axes[3].set_title('Rotated Cell Image [filtered emitters=%d, scatter opacity=%.2f]' % (len(cell.sub_subset), alpha))
